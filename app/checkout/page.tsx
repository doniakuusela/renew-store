...
export default function Checkout() {
  ...
  async function handlePayment() {
    ...
    try {
      ...
      const { data: pending, error: pendingError } = await supabase.from('pending_orders').insert({
        buyer_email: email,
      ...
      console.log('Pending order result:', pending, pendingError)
        ...
        <div style={{background:'white', padding:'40px', borderRadius:'4px', boxShadow:'0 4px 24px rgba(0,0,0,0.08)'}}>
          ...
          {product && (
            <div style={{background:'#EBF2EC', padding:'16px', borderRadius:'4px', marginBottom:'24px', display:'flex', justifyContent:'space-between', alignItems:'center'}}>
              <div>
                <div style={{fontSize:'13px', fontWeight:'500', color:'#1E1E1E'}}>{product.title}</div>
                <div style={{fontSize:'11px', color:'#7A7068', marginTop:'2px'}}>Seller: {product.seller}</div>
              </div>
              <div style={{fontSize:'22px', fontWeight:'600', color:'#2D5A3D', whiteSpace:'nowrap', marginLeft:'16px'}}>QAR {product.amount?.toLocaleString()}</div>
            </div>
          )}
          ...
          <div style={{marginBottom:'16px'}}>
            ...
            <input value={name} onChange={e => setName(e.target.value)} style={{width:'100%', border:'1.5px solid #D9CEBC', padding:'11px 14px', fontSize:'14px', outline:'none', boxSizing:'border-box', borderRadius:'2px'}} placeholder="Your name"/>
          </div>
          <div style={{marginBottom:'16px'}}>
            <label style={{display:'block', fontSize:'11px', fontWeight:'600', textTransform:'uppercase', color:'#7A7068', marginBottom:'6px'}}>Email</label>
            <input type="email" value={email} onChange={e => setEmail(e.target.value)} style={{width:'100%', border:'1.5px solid #D9CEBC', padding:'11px 14px', fontSize:'14px', outline:'none', boxSizing:'border-box', borderRadius:'2px'}} placeholder="your@email.com"/>
          </div>
          <div style={{marginBottom:'24px'}}>
            ...
            <input type="tel" value={phone} onChange={e => setPhone(e.target.value)} style={{width:'100%', border:'1.5px solid #D9CEBC', padding:'11px 14px', fontSize:'14px', outline:'none', boxSizing:'border-box', borderRadius:'2px'}} placeholder="+974 XXXX XXXX"/>
          </div>
          ...
          <button onClick={handlePayment} disabled={loading} style={{width:'100%', background:'#2D5A3D', color:'white', border:'none', padding:'15px', fontSize:'13px', fontWeight:'500', textTransform:'uppercase', cursor:'pointer', borderRadius:'2px'}}>
            {loading ? 'Processing...' : `Pay QAR ${product?.amount?.toLocaleString() || '0'} Securely`}
          </button>
